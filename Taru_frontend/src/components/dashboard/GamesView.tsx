...
const PHASE_CONFIG: Record<Phase, { emoji: string; label: string; sub: string }> = {
...
function BreathingBubble({ onBack }: { onBack: () => void }) {
  ...
  const remainingSec = Math.max(0, Math.ceil((sessionLen * 1000 - elapsed) / 1000))
  ...
  if (completed) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center rounded-3xl p-8 transition-colors duration-1000"
        style={{ background: '#DBEAFE' }}>
        <div className="text-center">
          <div className="text-5xl mb-4">🌿</div>
          <h2 className="text-2xl font-extrabold mb-2" style={{ color: '#1E3A5F' }}>Nice work!</h2>
          <p className="text-sm mb-6" style={{ color: '#3B6B9A' }}>
            You completed {sessionLen >= 60 ? `${sessionLen / 60} minute${sessionLen > 60 ? 's' : ''}` : `${sessionLen} seconds`} of mindful breathing.
          </p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => { setCompleted(false); start() }}
              className="px-5 py-2.5 rounded-xl text-sm font-bold text-white transition-all hover:opacity-90"
              style={{ background: '#1E3A5F' }}>
              Again
            </button>
            <button onClick={() => { setCompleted(false); setElapsed(0); onBack() }}
  ...
  if (started) {
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center rounded-3xl p-8 transition-colors duration-[2000ms]"
        style={{ background: bgColor }}>
        {/* Timer */}
        <div className="text-xs font-bold mb-6 tracking-widest" style={{ color: 'rgba(0,0,0,0.35)' }}>
        ...
        <div className="relative w-52 h-52 flex items-center justify-center mb-8">
          ...
          <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 200 200">
            <circle cx="100" cy="100" r={ringRadius} fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="3" />
            <circle cx="100" cy="100" r={ringRadius} fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth="3"
          ...
          <div className="absolute rounded-full flex items-center justify-center"
            style={{
              ...
              background: 'radial-gradient(circle at 35% 35%, rgba(255,255,255,0.9), rgba(147,197,253,0.5))',
              boxShadow: '0 0 60px rgba(147,197,253,0.4), inset 0 0 30px rgba(255,255,255,0.5)',
              transition: 'transform 0.1s linear',
            }}>
            <div className="text-center">
              <div className="text-3xl mb-1">{cfg.emoji}</div>
              <div className="text-sm font-extrabold" style={{ color: '#1E3A5F' }}>{cfg.label}</div>
            </div>
          </div>
        </div>

        <p className="text-sm font-medium mb-8" style={{ color: 'rgba(0,0,0,0.4)' }}>{cfg.sub}</p>
  ...
  return (
    <div className="space-y-6">
      <button onClick={onBack} className="text-sm hover:underline flex items-center gap-1" style={{ color: COLORS.fg3 }}>
      ...
      <div className="rounded-2xl p-8 border text-center" style={{ background: COLORS.card, borderColor: COLORS.border }}>
        ...
        <h2 className="text-xl font-extrabold mb-1" style={{ color: COLORS.fg }}>Breathing Bubble</h2>
        <p className="text-xs mb-6" style={{ color: COLORS.fg3 }}>
          Follow the bubble. Breathe in as it expands, hold, then breathe out as it shrinks.
        </p>
        ...
        <div className="flex justify-center gap-2 mb-6">
          {SESSION_OPTIONS.map(opt => (
            <button key={opt.seconds}
              ...
              style={{
                borderColor: sessionLen === opt.seconds ? COLORS.primary : COLORS.border,
                background: sessionLen === opt.seconds ? COLORS.primary : 'transparent',
...
export default function GamesView() {
  ...
  const games = [
    { id: 'breath', icon: <Wind size={20} />, title: 'Breathing Bubble', desc: 'Follow the bubble as it expands and shrinks. A calming breath exercise to reduce anxiety.', tag: 'Anxiety relief' },
